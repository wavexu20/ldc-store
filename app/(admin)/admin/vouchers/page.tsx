...
export default async function AdminVouchersPage() {
  const [batches, productOptions] = await Promise.all([
    ...
    db.query.products.findMany({ columns: { id: true, name: true, price: true }, with: { variants: { where: (variants, { eq }) => eq(variants.isActive, true), columns: { id: true, name: true, price: true } } }, orderBy: (table, { asc }) => [asc(table.name)] }),
  ]);
