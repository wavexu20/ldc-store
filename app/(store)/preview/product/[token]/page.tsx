...
export default async function ProductPreviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-6">
    ...
    {(contentHtml || product.images.length > 0) ? <><Separator className="my-6" /><div><h2 className="mb-3 font-medium">商品详情</h2>{product.images.length > 0 ? <ProductImageGallery productName={product.name} images={product.images} /> : null}{contentHtml ? <div className="prose prose-sm prose-zinc max-w-none dark:prose-invert [&_img]:mx-auto [&_img]:max-h-[720px] [&_img]:rounded-xl [&_video]:mx-auto [&_video]:w-full [&_video]:rounded-xl [&_video]:bg-black" dangerouslySetInnerHTML={{ __html: contentHtml }} /> : null}</div></> : null}
  </div>;
}
