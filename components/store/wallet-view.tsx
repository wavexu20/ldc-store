...
export function WalletView({ data }: { data: WalletData }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:py-10">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <Card className="overflow-hidden border-0 bg-slate-950 text-white shadow-xl dark:bg-slate-900">
        <CardHeader className="relative pb-4"><div className="absolute right-0 top-0 size-36 -translate-y-10 translate-x-10 rounded-full bg-indigo-500/25 blur-2xl" /><CardDescription className="relative text-slate-300">可用余额</CardDescription><CardTitle className="relative text-5xl font-semibold tracking-tight">¥{(availableCents / 100).toFixed(2)}</CardTitle><p className="relative mt-1 text-xs text-slate-400">会员编号 · {data.user.memberNo}</p></CardHeader>
        <CardContent className="relative grid grid-cols-2 gap-3"><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-xs text-slate-300">现金余额</p><p className="mt-1 text-lg font-medium">¥{(data.user.balanceCents / 100).toFixed(2)}</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-xs text-slate-300">奖励金</p><p className="mt-1 text-lg font-medium">¥{(data.user.bonusBalanceCents / 100).toFixed(2)}</p></div><div className="col-span-2 flex items-center justify-between border-t border-white/10 pt-4 text-sm"><span className="flex items-center gap-2 text-slate-300"><Coins className="size-4 text-amber-300" />会员积分</span><span className="font-semibold">{data.user.pointsBalance}</span></div></CardContent>
      </Card>
