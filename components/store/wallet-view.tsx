...
export function WalletView({ data }: { data: WalletData }) {
  ...
  const tierName = (key: MembershipTier["key"]) => t(tierNameKeys[key]);
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:py-10">
    <div className="flex flex-col gap-1"><h1 className="text-2xl font-semibold tracking-tight">{t("walletTitle")}</h1><p className="text-sm text-muted-foreground">{t("walletDescription")}</p></div>
    ...
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <Card className="overflow-hidden border-brand/15 bg-gradient-to-br from-brand/10 via-background to-brand/5 shadow-sm">
        <CardHeader className="relative pb-4"><div className="absolute right-0 top-0 size-36 -translate-y-10 translate-x-10 rounded-full bg-brand/15 blur-2xl" /><CardDescription className="relative text-muted-foreground">{t("availableBalance")}</CardDescription><CardTitle className="relative text-5xl font-semibold tracking-tight"><Money amount={availableCents} cents /></CardTitle><p className="relative mt-1 text-xs text-muted-foreground">{t("cnyAccountMember", { number: data.user.memberNo ?? "" })}</p></CardHeader>
        <CardContent className="relative grid grid-cols-2 gap-3"><div className="rounded-xl border border-brand/15 bg-card/80 p-3"><p className="text-xs text-muted-foreground">{t("cashBalance")}</p><p className="mt-1 text-lg font-medium">¥{(data.user.balanceCents / 100).toFixed(2)}</p></div><div className="rounded-xl border border-brand/15 bg-card/80 p-3"><p className="text-xs text-muted-foreground">{t("bonusBalance")}</p><p className="mt-1 text-lg font-medium">¥{(data.user.bonusBalanceCents / 100).toFixed(2)}</p></div><div className="col-span-2 flex items-center justify-between border-t border-brand/15 pt-4 text-sm"><span className="flex items-center gap-2 text-muted-foreground"><Coins className="size-4 text-warning" />{t("memberPoints")}</span><span className="font-semibold">{data.user.pointsBalance}</span></div><div className={`relative col-span-2 mt-1 overflow-hidden rounded-2xl p-4 shadow-sm ${tierVisual.card}`}><div className="absolute -right-10 -top-12 size-40 rounded-full border border-current/10" /><div className="absolute -bottom-16 right-16 size-36 rounded-full border border-current/10" /><div className="relative"><div className="flex items-start justify-between gap-3"><div className={`flex size-11 items-center justify-center rounded-xl p-1.5 ${tierVisual.iconWrap}`}><MembershipTierBadge tier={membership.tier.key} className="size-8" /></div><span className={`text-[10px] font-semibold tracking-[0.2em] ${tierVisual.detail}`}>G3D MEMBER</span></div><div className="mt-5 flex items-end justify-between gap-3"><div><p className="text-xl font-semibold tracking-tight">{tierName(membership.tier.key)}</p><p className={`mt-1 text-xs ${tierVisual.detail}`}>{t("lifetimeSpend", { amount: `¥${(membership.totalSpentCents / 100).toFixed(2)}` })}</p></div><p className={`font-mono text-xs ${tierVisual.detail}`}>{data.user.memberNo}</p></div><div className="mt-4"><div className={`mb-1.5 flex justify-between text-xs ${tierVisual.detail}`}><span>{membership.nextTier ? t("distanceToTier", { tier: tierName(membership.nextTier.key) }) : t("highestTier")}</span><span>{membership.nextTier ? t("amountRemaining", { amount: `¥${(membership.amountToNextCents / 100).toFixed(2)}` }) : t("unlocked")}</span></div><div className={`h-2 overflow-hidden rounded-full ${tierVisual.track}`} role="progressbar" aria-label={t("membershipProgress")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(membership.progress)}><div className={`h-full rounded-full transition-[width] duration-300 ${tierVisual.progress}`} style={{ width: `${membershipProgressWidth}%` }} /></div></div></div></div></CardContent>
      </Card>

      <Card className="border shadow-sm"><CardHeader className="pb-4"><CardTitle className="flex items-center gap-2 text-lg"><WalletCards className="size-5 text-brand" />{t("topupBalance")}</CardTitle><CardDescription>{t("topupBalanceHint")}</CardDescription></CardHeader><CardContent className="space-y-5"><div className="space-y-2"><Label htmlFor="amount">{t("topupAmount")}</Label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-lg font-medium text-muted-foreground">¥</span><Input id="amount" className="h-12 pl-8 text-lg font-semibold tabular-nums" type="number" min="1" step="0.01" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></div></div><div className="grid grid-cols-4 gap-2">{rechargeOptions.map((value) => <Button className="cursor-pointer" key={value} type="button" variant={amountCents === value * 100 ? "default" : "outline"} size="sm" onClick={() => setAmount(value.toFixed(2))}>¥{value}</Button>)}</div><div className="flex items-center gap-3 rounded-xl bg-success/10 px-3 py-2.5 text-sm text-success dark:text-success"><div className="rounded-lg bg-success/10 p-1.5"><Gift className="size-4" /></div><span className="flex-1">{t("topupBonusCurrent")}</span><strong>+¥{(bonusCents / 100).toFixed(2)}</strong></div><Button className="w-full cursor-pointer" size="lg" disabled={pending || amountCents < 100} onClick={recharge}>{pending ? <Loader2 className="animate-spin" /> : <CreditCard />}{t("topupNow")}</Button><p className="text-center text-xs text-muted-foreground">{t("topupAutoCredit")}</p>{externalStores.length > 0 ? <div className="border-t pt-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-sm font-medium">{t("externalPurchase")}</p><p className="mt-0.5 text-xs text-muted-foreground">{t("externalPurchaseHint")}</p></div></div><div className="grid grid-cols-3 gap-2">{externalStores.map((store) => <Button asChild className="text-xs" key={store.label} size="sm" variant="outline"><a aria-label={store.label} href={store.href} target="_blank" rel="noreferrer">{store.logo ? <><Image src={store.logo} alt={store.label} width={124} height={32} className="h-5 w-auto max-w-[92px] object-contain" /><span className="sr-only">{store.label}</span></> : store.label}<ExternalLink className="size-3 shrink-0" /></a></Button>)}</div></div> : null}</CardContent></Card>
    </div>

    <div className="grid gap-3 sm:grid-cols-2"><div className="flex items-center gap-3 rounded-xl border bg-card p-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-lg bg-warning/10 text-warning"><Gift className="size-5" /></div><div><p className="font-medium">{t("topupBenefit")}</p><p className="mt-1 text-sm text-muted-foreground">{t("topupBenefitDescription")}</p></div></div><div className="flex items-center gap-3 rounded-xl border bg-card p-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-lg bg-brand/10 text-brand"><Sparkles className="size-5" /></div><div><p className="font-medium">{t("pointsDiscount")}</p><p className="mt-1 text-sm text-muted-foreground">{t("pointsDiscountDescription")}</p></div></div></div>

    <Card><CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><History className="size-5" />{t("accountActivity")}</CardTitle><CardDescription className="mt-1">{t("accountActivityDescription")}</CardDescription></div></CardHeader><CardContent><Tabs defaultValue="balance"><TabsList><TabsTrigger value="balance">{t("balanceHistory")}</TabsTrigger><TabsTrigger value="benefits">{t("memberBenefits")}</TabsTrigger></TabsList><TabsContent value="balance" className="mt-4 divide-y">{data.transactions.length === 0 ? <EmptyState icon={<Landmark className="size-5" />} label={t("noBalanceRecords")} /> : data.transactions.map((item) => { const positive = item.amountCents > 0; return <div key={item.id} className="flex items-center gap-3 py-3"><div className={`rounded-full p-2 ${positive ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"}`}>{positive ? <ArrowDownLeft className="size-4" /> : <ArrowUpRight className="size-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.description || item.type}</p><p className="text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString(locale)}</p></div><div className="text-right"><p className={`font-semibold tabular-nums ${positive ? "text-emerald-600" : "text-rose-600"}`}>{positive ? "+" : ""}{(item.amountCents / 100).toFixed(2)}</p><p className="text-xs text-muted-foreground">{t("balanceAfter", { amount: (item.balanceAfterCents / 100).toFixed(2) })}</p></div></div>; })}</TabsContent><TabsContent value="benefits" className="mt-4 divide-y">{data.memberHistory.length === 0 ? <EmptyState icon={<Sparkles className="size-5" />} label={t("noBenefitRecords")} /> : data.memberHistory.map((item) => <div key={item.id} className="flex items-center gap-3 py-3"><div className={`rounded-full p-2 ${item.amount > 0 ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"}`}>{item.asset === "points" ? <Coins className="size-4" /> : <Gift className="size-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.description || item.type}</p><p className="text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString(locale)}</p></div><div className="text-right"><p className={item.amount > 0 ? "font-semibold text-emerald-600" : "font-semibold text-rose-600"}>{item.amount > 0 ? "+" : ""}{item.asset === "bonus" ? `¥${(item.amount / 100).toFixed(2)}` : item.amount}</p><p className="text-xs text-muted-foreground">{t("balanceLabel")} {item.asset === "bonus" ? `¥${(item.balanceAfter / 100).toFixed(2)}` : item.balanceAfter}</p></div></div>)}</TabsContent></Tabs></CardContent></Card>
  </div>;
}
